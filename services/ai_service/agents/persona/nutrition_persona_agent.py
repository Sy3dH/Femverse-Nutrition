...
class NutritionPersonaAgent(BaseAgent):
    ...
    DEFAULT_AGENT_NAME = AgentName.NUTRITION_PERSONA_UPDATE_SINGLE.value

    @staticmethod
    def _resolve_today(inputs: NutritionPersonaUpdateInput) -> str:
        try:
            dates = [
                entry.log_date
    ...
    @staticmethod
    def _sort_daily_log_ascending(inputs: NutritionPersonaUpdateInput) -> None:
        ...
        try:
            ...
            logger.warning(
                "NutritionPersonaAgent: failed to sort daily_log ascending; "
                "passing through original order"
            )

    @staticmethod
    def _resolve_prev_last_updated(inputs: NutritionPersonaUpdateInput) -> str:
        try:
